...
func main() {
	...
	flag.BoolVar(&reconnect, "rc", false, "Reconnect")
...
func connectToDB(ctx context.Context, cfg config, database string) (*sql.DB, error) {
	db, err := sql.Open("mysql", cfg.MySQLUri(database))
	if err != nil {
		return db, fmt.Errorf("failed to connect to db: %w", err)
	}
	...
	if err := db.PingContext(ctx); err != nil {
		return db, fmt.Errorf("failed to ping: %w", err)
	}
...
func initalizeDB(ctx context.Context, cfg config) error {
	db, err := connectToDB(ctx, cfg, "mysql")
	if err != nil {
